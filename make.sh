...
if [ ! -z ${comedi_eeros_source_dir+x} ]; then
  ...
  cmake -DCMAKE_TOOLCHAIN_FILE=$toolchain_file -DCMAKE_INSTALL_PREFIX=$install_dir \
	-DADDITIONAL_INCLUDE_DIRS="$install_dir/include" \
	-DADDITIONAL_LINK_DIRS="$install_dir/lib;$eeros_build_dir/libucl-build/" \
	-DCMAKE_BUILD_TYPE=Release $comedi_eeros_source_dir
  make
  make install
  popd
fi
...
if [ ! -z ${sim_eeros_source_dir+x} ]; then
  ...
  cmake -DCMAKE_TOOLCHAIN_FILE=$toolchain_file -DCMAKE_INSTALL_PREFIX=$install_dir \
	-DADDITIONAL_INCLUDE_DIRS="$install_dir/include" \
	-DADDITIONAL_LINK_DIRS="$install_dir/lib;$eeros_build_dir/libucl-build/" \
	-DCMAKE_BUILD_TYPE=Release $sim_eeros_source_dir
  make
  make install
  popd
fi
...
cmake	-DCMAKE_TOOLCHAIN_FILE=$toolchain_file -DCMAKE_INSTALL_PREFIX=$install_dir \
	-DADDITIONAL_INCLUDE_DIRS="$application_source_dir/include;$eeros_build_dir/libucl-src/include" \
	-DADDITIONAL_LINK_DIRS="$eeros_build_dir/libucl-build/" \
